...
class FrontEndController extends Controller
{
    ...
    public function edit($id) {
      ...
      /*  $kriteria = Kriteria::all();
        ...
        */
        ...
        return response()->json(['id'=>$profil[0]->id,'nama_lokasi' =>$profil[0]->nama_lokasi,'nilai' => $nilai]);
    ...
    public function profileAcuan() {
     ...
     public function perangkingan() {
        ...
        $sub_kriteria = Sub_kriteria::all();
        $profil = DB::table('profile')->get();
        $result = $this->resultprofilematching();
        return view('user/perangkingan', compact('kriteria', 'sub_kriteria', 'result'));

    }

    public function save(Request $r) {
 		$id = DB::table('profile')->insertGetId(
        	array('nama_lokasi' => $r->nama, 'user_id' => $r->user, 'status'=> $r->status)
    	);
    ...
    public function getProfile() {
    	...
    	$profile = DB::table('profile')->where('status', 'diinginkan')->get();
    ...
    public function getProfileDetail($id) {
    	$profile_detail = DB::table('profile_detail')
       			->select('kriteria.nama AS kriteria', 'kriteria.faktor', 'sub_kriteria.nama AS nama_sub_kriteria', 'sub_kriteria.nilai', 'profile_detail.profile_id')
    ...
    public function resultProfileMatching() {
        $no = 1;
        $data= [];
